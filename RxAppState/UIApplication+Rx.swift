...
private struct UserDefaultKeys {
    static let isFirstLaunchKey = "RxAppState_isFirstLaunch"
    static let firstLaunchOnlyKey = "RxAppState_firstLaunchOnly"
    static var numDidOpenAppKey = "RxAppState_numDidOpenApp"
    static var lastAppVersionKey = "RxAppState_lastAppVersion"
}
...
extension RxSwift.Reactive where Base: UIApplication {
    ...
    fileprivate var appVersions: (last: String, current: String) {
        return (last: UserDefaults.standard.string(forKey: UserDefaultKeys.lastAppVersionKey) ?? "",
    ...
    public var didOpenAppCount: Observable<Int> {
        return didOpenApp
            .map { _ in
                ...
                var count = userDefaults.integer(forKey: UserDefaultKeys.numDidOpenAppKey)
                count = min(count + 1, Int.max - 1)
                userDefaults.set(count, forKey: UserDefaultKeys.numDidOpenAppKey)
    ...
    public var isFirstLaunch: Observable<Bool> {
        return didOpenApp
            .map { _ in
                let userDefaults = UserDefaults.standard
                let didLaunchBefore = userDefaults.bool(forKey: UserDefaultKeys.isFirstLaunchKey)
                
                if didLaunchBefore {
                    return false
                } else {
                    userDefaults.set(true, forKey: UserDefaultKeys.isFirstLaunchKey)
                    userDefaults.synchronize()
                    return true
                }
        }
        ...
        return didOpenApp
            .map { _ in
                ...
                if lastAppVersion.isEmpty || lastAppVersion != currentAppVersion {
                    UserDefaults.standard.set(currentAppVersion, forKey: UserDefaultKeys.lastAppVersionKey)
